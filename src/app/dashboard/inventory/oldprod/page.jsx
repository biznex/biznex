    ...
    <DashboardLayout>
      <div className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-blue-500 text-white border border-black rounded-xl p-4 cursor-pointer" onClick={() => setShowAddProductForm(!showAddProductForm)}>
            Add Product
          </div>
          <div className="bg-blue-500 text-white border border-black rounded-xl p-4 cursor-pointer" onClick={() => setShowUpdateStockForm(!showUpdateStockForm)}>
            Update Stock/Product Details
          </div>
          <div className="bg-blue-500 text-white border border-black rounded-xl p-4">Export PDF</div>
          <div className="bg-blue-500 text-white border border-black rounded-xl p-4">Scan to Add Product</div>
          {showAddProductForm && (
            <div className="bg-white border border-black rounded-xl p-4 col-span-1">
              ...
              >
                ...
                <button type="button" className="w-full p-2 bg-gray-300 text-black rounded mt-2" onClick={() => setShowAddProductForm(false)}>
                  Cancel
                </button>
              </form>
            </div>
            ...
            <div className="bg-white border border-black rounded-xl p-4 col-span-1">
              ...
              >
                  ...
                  {filteredOptions.length > 0 && (
                    <ul className="border border-gray-300 rounded bg-white absolute z-10 w-full">
                      {filteredOptions.map((product) => (
                        <li key={product.id} className="p-2 cursor-pointer hover:bg-gray-200" onClick={() => handleSelectProduct(product)}>
                          {product.name}
                        </li>
                      ))}
                    </ul>
                ...
                <input
                  ...
                  placeholder={selectedProductDetails ? selectedProductDetails.stock : "New Stock Quantity"}
                  className="w-full mx-auto p-2 border rounded mb-2 text-center"
                  style={{ width: 'calc(100%)' }}
                />
                <select
                  ...
                  className="w-full mx-auto p-2 border rounded mb-2 text-center"
                  style={{ width: 'calc(100%)' }}
                  defaultValue={selectedProductDetails ? selectedProductDetails.category : ""}
                >
                  <option value="">Select Category</option>
                  {categories.map((cat) => (
                    <option key={cat} value={cat}>
                ...
                <input
                  type="text"
                  name="sku"
                  placeholder={selectedProductDetails ? selectedProductDetails.sku : "SKU"}
                  className="w-full mx-auto p-2 border rounded mb-2 text-center"
                  style={{ width: 'calc(100%)' }}
                />
                <button type="submit" className="w-full p-2 bg-blue-500 text-white rounded">
                ...
                <button type="button" className="w-full p-2 bg-gray-300 text-black rounded mt-2" onClick={() => setShowUpdateStockForm(false)}>
                  Cancel
                </button>
              </form>
            </div>
        ...
        <div className="mt-4 bg-white border border-black rounded-xl p-4">
          <div className="flex space-x-2 mb-4 justify-around">
            <button className={`p-2 border rounded ${filter === 'all' ? 'bg-blue-500 text-white' : 'bg-white'}`} style={{ width: '20%' }} onClick={() => setFilter('all')}>
              All
            </button>
            <button className={`p-2 border rounded ${filter === 'active' ? 'bg-blue-500 text-white' : 'bg-white'}`} style={{ width: '20%' }} onClick={() => setFilter('active')}>
              Active
            </button>
            <button className={`p-2 border rounded ${filter === 'non-active' ? 'bg-blue-500 text-white' : 'bg-white'}`} style={{ width: '20%' }} onClick={() => setFilter('non-active')}>
              Inactive
            </button>
            <button className={`p-2 border rounded ${filter === 'low-stock' ? 'bg-blue-500 text-white' : 'bg-white'}`} style={{ width: '20%' }} onClick={() => setFilter('low-stock')}>
              Low Stock
            </button>
              ...
              {currentItems.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="py-2 border border-slate-800 p-2 text-left">
                    {product.status === 'active' ? <span className="bg-green-500 text-white rounded-md p-1">Active</span> : <span className="bg-red-500 text-white rounded-md p-1">Inactive</span>}
                  </td>
                  <td className="py-2 border border-slate-800 p-2 text-left">
                    {product.stock <= 10 ? <span className="bg-red-500 text-white rounded-md p-1">{product.stock}</span> : product.stock}
                  </td>
          ...
          <div className="flex justify-center mt-4">
            ...
            <div className="flex space-x-2">
              {pageNumbers.map((number) => (
                <button key={number} onClick={() => handlePageChange(number)} className={`p-2 border rounded ${currentPage === number ? 'bg-blue-500 text-white' : 'bg-white'}`}>
                  {number}
                </button>
              ))}
            </div>
