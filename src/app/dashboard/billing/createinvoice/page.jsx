...
function DashboardPage() {
    ...
    const exportToPDF = () => {
        const errors = {};
        if (!order.order_name) errors.order_name = 'Order Name is required';
        if (!order.created_at) errors.created_at = 'Created At is required';
        if (!order.to_business_name) errors.to_business_name = 'To Business Name is required';
        if (!order.to_business_address) errors.to_business_address = 'To Business Address is required';
        if (!order.from_business_name) errors.from_business_name = 'From Business Name is required';
        if (!order.from_business_address) errors.from_business_address = 'From Business Address is required';
        if (order.line_items.some(item => !item.title || !item.quantity || !item.unit_price)) {
            errors.line_items = 'All line items must have title, quantity, and unit price.';
        }
            ...
            <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
                <div style={{ marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <input type="text" name="order_name" placeholder="Order Name" value={order.order_name} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.order_name ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.order_name && <p style={{ color: 'red' }}>{formErrors.order_name}</p>}
                        <input type="date" name="created_at" value={order.created_at} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.created_at ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.created_at && <p style={{ color: 'red' }}>{formErrors.created_at}</p>}
                        <input type="text" name="to_business_name" placeholder="To Business Name" value={order.to_business_name} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.to_business_name ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.to_business_name && <p style={{ color: 'red' }}>{formErrors.to_business_name}</p>}
                        <input type="text" name="to_business_address" placeholder="To Business Address" value={order.to_business_address} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.to_business_address ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.to_business_address && <p style={{ color: 'red' }}>{formErrors.to_business_address}</p>}
                        <input type="text" name="from_business_name" placeholder="From Business Name" value={order.from_business_name} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.from_business_name ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.from_business_name && <p style={{ color: 'red' }}>{formErrors.from_business_name}</p>}
                        <input type="text" name="from_business_address" placeholder="From Business Address" value={order.from_business_address} onChange={(e) => handleInputChange(e)} style={{ padding: '8px', border: formErrors.from_business_address ? '1px solid red' : '1px solid #ddd' }} />
                        {formErrors.from_business_address && <p style={{ color: 'red' }}>{formErrors.from_business_address}</p>}

                        <h3>Line Items</h3>
                        {order.line_items.map((item, index) => (
                            <div key={item.id} style={{ display: 'flex', gap: '10px', marginBottom: '5px' }}>
                                <input type="text" name="title" placeholder="Prod/Service" value={item.title} onChange={(e) => handleInputChange(e, index)} style={{ padding: '8px', border: formErrors.line_items && !item.title ? '1px solid red' : '1px solid #ddd', maxWidth: '150px' }} />
                                <input type="number" name="quantity" placeholder="Quantity" value={item.quantity} onChange={(e) => handleInputChange(e, index)} style={{ padding: '8px', border: formErrors.line_items && !item.quantity ? '1px solid red' : '1px solid #ddd', maxWidth: '80px' }} />
                                <input type="number" name="unit_price" placeholder="Unit Price" value={item.unit_price} onChange={(e) => handleInputChange(e, index)} style={{ padding: '8px', border: formErrors.line_items && !item.unit_price ? '1px solid red' : '1px solid #ddd', maxWidth: '100px' }} />
                        ...
                        {formErrors.line_items && <p style={{ color: 'red' }}>{formErrors.line_items}</p>}
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <h1 style={{ textAlign: 'center', paddingLeft: '10px', fontSize: '3rem' }}>Invoice</h1>
                </div>
